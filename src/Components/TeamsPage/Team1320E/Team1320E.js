...
class Team1320E extends Component {
  ...
  componentDidMount () {
    this.fetchSkillRank('https://api.vexdb.io/v1/get_skills?season_rank=true&team=1320E&season=current&type=2')
      .then(response => this.setState({ robotSkillRank: response }))
    this.fetchSkillRank('https://api.vexdb.io/v1/get_skills?season_rank=true&team=1320E&season=current&type=0')
      .then(response => this.setState({ driveSkillRank: response }))
    this.fetchSkillRank('https://api.vexdb.io/v1/get_skills?season_rank=true&team=1320E&season=current&type=1')
      .then(response => this.setState({ programSkillRank: response }))
    this.fetchAwardNum('https://api.vexdb.io/v1/get_awards?team=1320E')
      .then(response => this.setState({ awardNum: response }))
    this.fetchWorldQual('https://api.vexdb.io/v1/get_events?team=1320E')
      .then(response => this.setState({ worldQual: response }))
    this.fetchStateQual('https://api.vexdb.io/v1/get_events?team=1320E')
